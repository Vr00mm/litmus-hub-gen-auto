...
func generateExperiments(composants *resources.Resources, experimentsList []string) []types.Experiment {
	...
	for _, experimentName := range experimentsList {
		for composant := range composants.Objects {
			...
			for _, experiment := range exps {
				...
				composants.Objects[composant].AddGeneratedExperiment(experiment.Name)
			}
		}
	}
	return experiments
...
func generateHub(clusterName string, namespace *resources.Resources, experiments []types.Experiment, workflows []types.Workflow) types.Manifest {
